...
def list_images(directory):
    ...
    for dir0 in dir:
        ...
        for dir1 in os.listdir(os.path.join(directory, dir0)):
            print("die=", dir0)
            print(dir1)
            req_path = os.path.join(directory, dir0, dir1)
...
def get_image(path, height=256, width=256, flag=False):
    ...
    if height is not None and width is not None:
        image = numpy.array(Image.fromarray(image).resize([height, width], resample=PIL.Image.BICUBIC))
    return image
...
def save_image_test(img_fusion, output_path):
    ...
    img_fusion = img_fusion.resize((256, 256), resample=Image.BICUBIC)
